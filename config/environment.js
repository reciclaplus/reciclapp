/**
 * Centralized Environment Configuration
 * 
 * This module provides a unified way to access environment-specific configuration
 * for both development and production environments.
 */

// Get the current environment
const NODE_ENV = process.env.NODE_ENV || 'development';

// Environment-specific configuration
const config = {
  // Current environment
  environment: NODE_ENV,
  
  // API URLs
  apiUrl: process.env.NEXT_PUBLIC_API_URL || (NODE_ENV === 'production' 
    ? 'https://fastapi-dot-norse-voice-343214.uc.r.appspot.com' 
    : 'http://localhost:8000'),
  
  // OAuth Configuration
  oauthRedirectUri: process.env.OAUTH_REDIRECT_URI || (NODE_ENV === 'production'
    ? 'https://sabanayegua.reciclaplus.com'
    : 'http://localhost:3000'),
  
  // Firebase/Firestore Configuration
  firebase: {
    projectId: process.env.FIREBASE_PROJECT_ID || (NODE_ENV === 'production'
      ? 'norse-voice-343214'
      : 'reciclapp-dev-23776'),
    databaseId: process.env.FIRESTORE_DATABASE_ID || '(default)',
  },
  
  // Google Cloud
  googleCloud: {
    projectId: process.env.GOOGLE_CLOUD_PROJECT || (NODE_ENV === 'production'
      ? 'norse-voice-343214'
      : 'reciclapp-dev-23776'),
  },
  
  // CORS Configuration
  allowedOrigins: process.env.ALLOWED_ORIGINS 
    ? process.env.ALLOWED_ORIGINS.split(',')
    : (NODE_ENV === 'production'
      ? ['https://reciclapp-dev-dot-norse-voice-343214.uc.r.appspot.com', 'https://sabanayegua.reciclaplus.com', 'http://localhost:3000']
      : ['http://localhost:3000', 'http://127.0.0.1:3000']),
    
  // File paths (mainly for backend)
  files: {
    firestoreServiceAccount: process.env.FIRESTORE_SERVICE_ACCOUNT_FILE || (NODE_ENV === 'production'
      ? './routers/firestore-service-account.json'
      : './routers/firestore-service-account-dev.json'),
    clientSecret: process.env.CLIENT_SECRET_FILE || (NODE_ENV === 'production'
      ? './client_secret_.json'
      : './client_secret_dev.json'),
  },
  
  // Helper functions
  isDevelopment: () => NODE_ENV === 'development',
  isProduction: () => NODE_ENV === 'production',
  
  // Firestore collections prefix for environment separation
  getCollectionName: (baseName) => {
    const prefix = NODE_ENV === 'production' ? '' : 'dev_';
    return `${prefix}${baseName}`;
  }
};

// Update the existing configuration to use environment-aware API URL
const API_URL = config.apiUrl;

// Keep the existing conf object for backward compatibility, 
// but make it extensible for multiple environments
const conf = {
  sabanayegua: {
    nombre: 'Sabana Yegua',
    file: 'Puntos_SY.json',
    map_center: { lat: 18.4606607, lng: -70.8405734 },
    categories: [
      { value: 'casa', label: 'Casa Particular' },
      { value: 'escuela', label: 'Escuela' },
      { value: 'negocio', label: 'Negocio' },
      { value: 'iglesia', label: 'Iglesia' }
    ],
    comunidades: [
      { nombre: 'Sabana Yegua', center: '18.4606607,-70.8405734', barrios: ['Barrio Blanco', 'Barrio Pintado', 'Barrio Nuevo', 'Ojo de Agua', 'San Francisco', 'El Abanico', 'Los Cartones', 'Barrio Tranquilo', 'Las Mercedes'] },
      { nombre: 'Km 13', center: '18.453435,-70.853577', barrios: ['Km 13'] },
      { nombre: 'Km 15', center: '18.463566,-70.868628', barrios: ['Km 15'] },
      { nombre: 'Nature Village', center: '18.7146767,-71.0251952', barrios: ['Nature Village'] },
      { nombre: 'Proyecto 2C', center: '18.3760099,-70.8560015', barrios: ['Proyecto 2C'] },
      { nombre: 'Los Negros', center: '18.3573766,-70.8458091', barrios: ['Los Negros'] },
      { nombre: 'Barrera', center: '18.322873,-70.901599', barrios: ['Barrera'] },
      { nombre: 'Km 8', center: '18.4413061,-70.8126939', barrios: ['Km 8'] }
    ],
    barrios: [
      { nombre: 'Barrio Blanco', color: '#FF95C5', center: '18.4562141,-70.829' },
      { nombre: 'Barrio Pintado', color: '#8884d8', center: '18.4579091,-70.835' },
      { nombre: 'Barrio Nuevo', color: '#82ca9d', center: '18.4600755,-70.8293113' },
      { nombre: 'Ojo de Agua', color: '#FFC898', center: '18.4608084,-70.8335' },
      { nombre: 'San Francisco', color: '#916BBF', center: '18.4640215,-70.8315343' },
      { nombre: 'El Abanico', color: '#ffff94', center: '18.4629935,-70.8337693' },
      { nombre: 'Los Cartones', color: '#C8C6C6', center: '18.4604671,-70.8372607' },
      { nombre: 'Barrio Tranquilo', color: '#FF5C58', center: '18.4649761,-70.8411297' },
      { nombre: 'Las Mercedes', color: '#5D5B5B', center: '18.4671061,-70.8334927' },
      { nombre: 'Km 8', color: '#41521F', center: '18.4413061,-70.8126939' },
      { nombre: "Km 13", color: "#000000", center: "18.453435,-70.853577" },
      { nombre: "Km 15", color: "#000000", center: "18.463566,-70.868628" },
      { nombre: "Nature Village", color: "#000000", center: "18.7146767,-71.0251952" },
      { nombre: "Proyecto 2C", color: "#000000", center: "18.3760099,-70.8560015" },
      { nombre: "Los Negros", color: "#000000", center: "18.3573766,-70.8458091" },
      { nombre: "Barrera", color: "#000000", center: "18.322873,-70.901599" }
    ]
  },
  proyecto4: {
    nombre: 'Proyecto 4',
    file: 'Puntos_P4.json',
    barrios: [
      { nombre: 'P4 Viejo Oeste', color: '#FF95C5', center: '18.412643,-70.828455' },
      { nombre: 'P4 Viejo Este', color: '#8884d8', center: '18.412103,-70.823498' },
      { nombre: 'P4 Nuevo Oeste', color: '#82ca9d', center: '18.421417,-70.829368' },
      { nombre: 'P4 Nuevo Este', color: '#FFC898', center: '18.420669,-70.824762' }
    ],
    map_center: { lat: 18.416482964779416, lng: -70.82662731805073 }
  },
  sample: {
    nombre: 'Sabana Yegua',
    file: 'sample.json',
    map_center: { lat: 18.4606607, lng: -70.8405734 },
    categories: [
      { value: 'casa', label: 'Casa Particular' },
      { value: 'escuela', label: 'Escuela' },
      { value: 'negocio', label: 'Negocio' },
      { value: 'iglesia', label: 'Iglesia' }
    ],
    barrios: [
      { nombre: 'Barrio Blanco', color: '#FF95C5', center: '18.4562141,-70.829' },
      { nombre: 'Barrio Pintado', color: '#8884d8', center: '18.4579091,-70.835' },
      { nombre: 'Barrio Nuevo', color: '#82ca9d', center: '18.4600755,-70.8293113' },
      { nombre: 'Ojo de Agua', color: '#FFC898', center: '18.4608084,-70.8335' },
      { nombre: 'San Francisco', color: '#916BBF', center: '18.4640215,-70.8315343' },
      { nombre: 'El Abanico', color: '#ffff94', center: '18.4629935,-70.8337693' },
      { nombre: 'Los Cartones', color: '#C8C6C6', center: '18.4604671,-70.8372607' },
      { nombre: 'Barrio Tranquilo', color: '#FF5C58', center: '18.4649761,-70.8411297' },
      { nombre: 'Las Mercedes', color: '#5D5B5B', center: '18.4671061,-70.8334927' }
    ]
  }
};

// Export for ES6 modules and CommonJS
module.exports = { config, API_URL, conf };
module.exports.default = config;