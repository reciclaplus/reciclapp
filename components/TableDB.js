...
export default function Table(props) {
  ...
  useEffect(() => {
    ...
    const recogida = fetch('./api/db/get-recogida',{
      method: "GET"
    ...
    .then(recogida => {
      ...
      for (let pdr in tableData){
        ...
        var userId = tableData[pdr].id
        var userBarrio = tableData[pdr].barrio
        var userData = recogida.filter(({id, barrio, value}) => ((id === userId) && (barrio === userBarrio)))
        ...
        if (userData.length < 3){
          ...
          if (userData.filter(({value}) => (value === "si")).length === 3){
            alerta = false
          }
          else{
            alerta = true
