...
async def last_n(current_user: Annotated[User, Depends(require_permission(Permissions.READ_RECOGIDA))], n: int = 5):
...
async def get_individual_id_week(
    current_user: Annotated[User, Depends(require_permission(Permissions.READ_RECOGIDA))], year: int, week: int, id: str
):
    if week < 10:
        week = f"0{week}"
...
async def get_week(
    current_user: Annotated[User, Depends(require_permission(Permissions.READ_RECOGIDA))], year: int, week: int
):
    if week < 10:
        week = f"0{week}"
    doc = db.collection("recogida").document(f"{year}{week}").get()
...
async def set_week(
    current_user: Annotated[User, Depends(require_permission(Permissions.WRITE_RECOGIDA))],
    year: int,
    week: int,
    recogida: dict,
):
...
async def last_n_by_barrio(
    current_user: Annotated[User, Depends(require_permission(Permissions.READ_RECOGIDA))],
    n: int = 5,
    category: str = None,
    barrio: str = None,
):
...
async def get_weight(current_user: Annotated[User, Depends(require_permission(Permissions.READ_WEIGHT))]):
...
async def set_weight(
    current_user: Annotated[User, Depends(require_permission(Permissions.WRITE_WEIGHT))], id: int, new_weight: dict
):
...
async def delete_weight(current_user: Annotated[User, Depends(require_permission(Permissions.WRITE_WEIGHT))], id: int):
